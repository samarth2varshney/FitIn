package com.example.fitin.ui.feed

import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel

class FeedViewModel : ViewModel() {

//    // Define a MutableLiveData list
//    private val _feedList = MutableLiveData<List<FeedItem>>()
//
//    // Expose the list as LiveData
//    val feedList: LiveData<List<FeedItem>> get() = _feedList
//
//    // Function to set/update the list
//    fun setFeedList(newList: List<FeedItem>) {
//        _feedList.value = newList
//    }
//
//    // Optionally, you can provide a method to update the list
//    fun addFeedItem(item: FeedItem) {
//        _feedList.value = _feedList.value?.toMutableList()?.apply {
//            add(item)
//        }
//    }

}